...
def fit1(field, myData, selected_ly, u=1.00, v=1.00, z=1.00):
    ...
    for i, case in enumerate(CaseList.keys()):
        ...
        try:
            ...
            ax.plot(x_fit, y_fit, color=colors[1], linestyle='-', linewidth=1.25, label=f'FHWA-Haversine, $R^2$:{R2:.3f} ')
            ax.scatter([x_fit[0], matching_x], [y_fit[0], matching_y], color=colors[1],  marker=markers[1], s=30)
