...
def fit2(field, myData, selected_ly):
    
    def new_function(t, d):
        return np.sin(np.pi / 2 + np.pi * t / d) ** 2
    ...
    for i, case in enumerate(CaseList.keys()):
        ...
        peak_idx = np.argmax(y_data_norm)
        d = 2 * abs(x_data[peak_idx]-x_data[0])
        print(f'Calculated d: {d}')
        tex = abs(x_data[peak_idx]-x_data[0]) # Extending time steps

        # Fitting the new function
        try:
            popt, _ = curve_fit(lambda t, d: new_function(t, d), x_data, y_data_norm, p0=[d], maxfev=10000)
            ...
            #find the first indez at which y_fit is zero
            init_index = np.where(y_fit < 0.001)[0][0]
            
            peak_value = y_fit[peak_index]
            init_value = x_fit[init_index]
            ...
            tsteps = matching_x - x_fit[init_index ]
            print(f'{case}: t={tsteps:.3f} sec, f={1/(2*np.pi*tsteps):.3f} Hz')

            ax.plot(x_fit, y_fit, color=colors[2], linestyle='-', linewidth=1.25, label=f'Haversine, $R^2$:{R2:.3f}')
            ax.scatter([x_fit[init_index ], matching_x], [y_fit[init_index ], matching_y], color=colors[2],  marker=markers[1], s=30)
