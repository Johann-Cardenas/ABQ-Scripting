...
guesstimate = [[1.0, 2.0, -1.0]] 

            
# Longitudinal Profile Along the Z Axis
def fit0(field, myData, selected_ly, u=1.00, v=1.00, z=1.00):
    ...
    for i, case in enumerate(CaseList.keys()):
        ...
        ax.scatter(x_data, y_data_norm, color=colors[0], marker=markers[0], s=30, label='Normalized Data')
        
        # Fitting the haversine function
        initial_guess = guesstimate[i]
        bounds = ([0, 0, -np.inf], [10, 100, np.inf])
        tex = x_data[-1]  # Extending time steps
        ...
        try:
            ...
            print(f'Fitting Parameters {case}: b1={popt[0]:.3f}, b2={popt[1]:.3f}, b3={popt[2]:.3f}')
            # Edit manually values of popt to get a better fit
            popt[0] = u * popt[0] 
            popt[1] = v * popt[1]
            popt[2] = z * popt[2]
            
            # Calculate the pulse duration
            x_fit = np.linspace(x_data[0], x_data[-1]+ x_data[-1], 1000)
            ...
            t_RH = np.abs(Xt - X0) * 2
            print(f'Fitting Parameters {case}: t_RH={t_RH:.3f}, b1={popt[0]:.3f}, b2={popt[1]:.3f}, b3={popt[2]:.3f}')
            ...
            ax.plot(x_fit, y_fit, color=colors[4], linestyle='-', linewidth=1.25, label=f'FHWA-Haversine (M), $R^2$:{R2:.3f} ')
            ax.scatter([x_fit[0], matching_x], [y_fit[0], matching_y], color=colors[4],  marker=markers[1], s=30)
...
fit0('S22', myData_layers, 'AC3', u=1.20, v=0.85)
