...
for i, case in enumerate(Cases):
    ...
    x0_index = np.argmin(np.abs(xi[Xcoord, :]))
    ...
    ax_profile.plot(yi[:, x0_index], z_profile, linewidth=2.5, color=colors[i], linestyle=linestyles[i], label=labels[i])
    
    

# Set the labels for each axis and adjust their positions
xlabel_profile = ax_profile.set_xlabel('Width (mm)', labelpad=7.5)
